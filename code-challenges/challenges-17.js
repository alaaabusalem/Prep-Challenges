...
const recursionPattern = (int1, int2) => {
 ...
 let outarry=[];
  function findArray(int1, int2){
    outarry.push(int1);
    if(int1>=0){
   findArray(int1-int2,int2)
   outarry.push(int1);
    }
  }
  findArray(int1,int2);
...
const filterLinks = (str) => {
    const re = new RegExp("www.([a-zA-Z0-9])+.(com|org|net)")
    let firstIndex=str.search(re);
    //console.log(firstInext);
    //let lastIndex=str.lastIndexOf(re);
    //console.log(lastIndex);
    let arry=str.split("");
    let url="";
    for(let i=firstIndex;i<arry.length;i++){
        if(url.search(".com")===-1){
            if(url.search(".org")===-1){
                if(url.search(".net")===-1){
                    url=url+arry[i];
